...
def show_venue(venue_id):
  ...
  query = query.join(Venue_Genre)
  venue_results = query.filter(Venue.id == venue_id).all()
  genres = [venue_info[2] for venue_info in venue_results]

  # Find past shows
  query = db.session.query(Artist.id, Artist.name, Artist.image_link, 
                           Show.show_time)
  query = query.join(Show).join(Venue)
  show_results = query.filter(Venue.id == venue_id, 
                              Show.show_time < datetime.datetime.now()).order_by(Show.show_time.desc()).all()
  past_shows = []
  for show_info in show_results:
    show_dict = {
      "artist_id": show_info[0],
      "artist_name": show_info[1],
      "artist_image_link": show_info[2],
      "start_time": datetime.datetime.strftime(show_info[3], '%Y-%m-%d %I:%M%p')
    }
  ...
  query = query.join(Show).join(Venue)
  show_results = query.filter(Venue.id == venue_id, 
                              Show.show_time > datetime.datetime.now()).order_by(Show.show_time).all()
  upcoming_shows = []
  for show_info in show_results:
    show_dict = {
      "artist_id": show_info[0],
